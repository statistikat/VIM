...
# single sweep (sequential = FALSE) keeps the vignette build short
result_xgboost <- vimpute(
  data = data,
  method = setNames(as.list(rep("xgboost", ncol(data))), names(data)),
  pred_history = TRUE, verbose = FALSE, sequential = FALSE
  )
...
result_regularized <- vimpute(
  ...
  pred_history = TRUE, sequential = FALSE
  )
...
# Perform imputation (single sweep keeps the vignette build short)
result <- vimpute(data = df, pred_history = TRUE, sequential = FALSE)
