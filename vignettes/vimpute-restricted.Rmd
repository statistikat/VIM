...
has_restricted_backend <- requireNamespace("validate", quietly = TRUE) &&
  ...
  # vimpute() itself runs on the mlr3 stack, which VIM suggests rather than
  # imports; without it there is no restricted backend to demonstrate.
  all(vapply(c("mlr3", "mlr3learners", "mlr3pipelines", "mlr3tuning",
               "paradox", "R6", "future"),
...
synthetic business-statistics example with many linear edit rules. The rules
ship as plain data frames of rule text (so the data set does not depend on
`validate`); `validate::validator(.data = )` turns a rule set into a validator.
The test suite uses this data at larger scale; the vignette uses a smaller
