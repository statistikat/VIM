...
`m = 5` imputations; the default `uncert = "pmm"` draws from observed donor
values, so the imputations differ between runs (a prerequisite for Rubin's
rules). Per-variable settings use the spec interface; three sequential
iterations give the convergence chains something to show.
...
evaluation(truth, completed, m = attr(amp, "where"))
```
