...
Three configurations of `vimpute(method = "robust", m = M)`:

* **default**: `uncert = "pmm"` — stochastic donor draws (VIM's 7.3.0
  default for numeric variables);
...
```{r, eval = run_mice}
configs <- list(
  "default (pmm)"      = list(boot = FALSE, uncert = "pmm"),
...
paper-scale `NSIM`. The default `uncert = "pmm"` and the textbook
`boot + normalerror` configurations produce pooled SEs of the right size.
