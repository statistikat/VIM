...
In addition to [Model based Imputation Methods](http://statistikat.github.io/VIM/articles/modelImp.html) the `VIM` package also presents donor based imputation methods, namely Hot-Deck Imputation, k-Nearest Neighbour Imputation and fast matching/imputation based on categorical variable.
...
For `matchImpute()` suitable donars are searched based on matching of the categorical variables. 
...
The specified variables in `matchImpute()` serve as a donar and enable imputation for `NonD`.
...
`matchImpute()` works by sampling values from the suitable donars and also provide reasonable results.
```{r, fig.height=5}
...
The plot indicates that all missing values have been imputed by the kNN algorithm. The following table displays the rounded first five results of the imputation for all variables.  
