...
show_test("3) vim_complete() auf vimmi")
d1 <- vim_complete(res2, 1L)
dall <- vim_complete(res2, "all")
dlong <- vim_complete(res2, "long")

check(is.data.frame(d1), "vim_complete(res, 1) sollte einen Datensatz liefern.")
check(length(dall) == 3L, "vim_complete(res, 'all') sollte drei Datensaetze liefern.")
check(nrow(dlong) == 3L * nrow(dt2), "vim_complete(res, 'long') sollte alle Imputationen stapeln.")
