...
#' @param hier_cat hierachichal strucuture of cat variables (list of length=length cat_vars)
...
#'   hier_cat=list(NULL,c(2,2)),epochs=1, decimal_points = 3)
...
transformerImpute <- function(data,target,cat_vars=NULL,hier_cat=NULL,model=NULL,
  ...
  if(!is.null(hier_cat)&!is.null(cat_vars)){
    stopifnot(length(hier_cat)==length(cat_vars))
  }
...
generate_tok <- function(dat_str,lens,cat_vars=NULL, hier_cat = NULL){
  ...
  for(i in seq_along(names(dat_str))){
    if(names(dat_str)[i] %in% cat_vars){
      ...
      if(!is.null(hier_cat)){
        if(!is.null(hier_cat[[which(cat_vars==names(dat_str)[i])]])){
          start_index <- 1
          unX <- list()
          tmphc <- hier_cat[[which(cat_vars==names(dat_str)[i])]]
...
data_to_text <- function(dat,cat_vars,hier_cat,target,decimal_points_target,decimal_points,round_long_decs){
  ...
  for(i in 1:ncol(dat)){
    if(names(dat)[i] %in% cat_vars){
      ...
      len[[names(dat)[i]]] <- 1
      if(!is.null(hier_cat)){
        if(!is.null(hier_cat[[which(cat_vars==names(dat)[i])]])){
          len[[names(dat)[i]]] <- length(hier_cat[[which(cat_vars==names(dat)[i])]])
        }
      }
